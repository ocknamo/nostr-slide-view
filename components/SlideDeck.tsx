...
export const SlideDeck: React.FC<SlideDeckProps> = ({ slides, onBack, nostrId, lang }) => {
  ...
  const exportAsJSON = () => {
    ...
    downloadFile(data, `nostr-slide-${nostrId.slice(0, 8)}.json`, "application/json");
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'e': setIsExportOpen(!isExportOpen); break;
      }
    };
  ...
  if (viewMode === 'grid') {
    return (
      <div className="fixed inset-0 z-[100] bg-nostr-dark p-8 overflow-y-auto animate-fade-in no-print" role="dialog" aria-modal="true" aria-labelledby="grid-title">
        <div className="flex justify-between items-center mb-8 sticky top-0 bg-nostr-dark/95 backdrop-blur z-20 py-4 border-b border-gray-800">
          <h2 id="grid-title" className="text-2xl font-bold text-white tracking-tight">{t.deck.gridTitle}</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
          {slides.map((slide, idx) => (
            <button 
              key={slide.id} 
              onClick={() => { setCurrentIndex(idx); setViewMode('slide'); }}
            ...
            >
              <img src={slide.imageUrl} alt="" className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
      ...
      >
        ...
        >
          ...
          <div className="flex gap-2 relative" ref={exportRef}>
             ...
             <button onClick={handleShare} className="p-3 bg-black/40 hover:bg-black/80 rounded-full text-white backdrop-blur-md transition border border-white/10 focus:ring-2 focus:ring-white" aria-label={t.deck.share}><Icons.Share aria-hidden="true" /></button>
             <button onClick={toggleFullScreen} className={`p-3 rounded-full text-white backdrop-blur-md transition border border-white/10 focus:ring-2 focus:ring-white ${isFullscreen ? 'bg-nostr-primary' : 'bg-black/40 hover:bg-black/80'}`} aria-label={isFullscreen ? t.deck.exitFullscreen : t.deck.fullscreen}><Icons.Maximize aria-hidden="true" /></button>
             <button onClick={() => setViewMode('grid')} className="p-3 bg-black/40 hover:bg-black/80 rounded-full text-white backdrop-blur-md transition border border-white/10 focus:ring-2 focus:ring-white" aria-label={t.deck.grid}><Icons.Grid aria-hidden="true" /></button>
             <button onClick={() => setShowNotes(!showNotes)} className={`p-3 rounded-full text-white backdrop-blur-md transition border border-white/10 focus:ring-2 focus:ring-white ${showNotes ? 'bg-nostr-primary' : 'bg-black/40 hover:bg-black/80'}`} aria-label={t.deck.notes} aria-pressed={showNotes}><Icons.Info aria-hidden="true" /></button>
          </div>
        </nav>

        <div aria-live="polite" className="absolute top-20 left-1/2 -translate-x-1/2 z-[60]">
          {shareStatus && (
        ...
        <div className="flex-1 relative overflow-hidden bg-black" aria-live="polite">
          <div 
            ...
            {slides.map((slide, idx) => (
              ...
              >
                ...
                {showNotes && slide.content && (
                  <div className={`absolute bottom-16 left-0 right-0 flex justify-center z-20 px-4 pointer-events-none transition-all duration-700 ${currentIndex === idx ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-1.5 bg-white/5 z-50" role="progressbar" aria-valuenow={progress} aria-valuemin={0} aria-valuemax={100} aria-label={t.deck.progress}>
          <div className="h-full bg-gradient-to-r from-nostr-primary to-fuchsia-500 transition-all duration-700" style={{ width: `${progress}%` }} />
        </div>
      </div>
    </>
  );
